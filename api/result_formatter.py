...
"""
        ...
        if query_analysis:
            ...
            md_content += f"- **Question Type:** {query_analysis.get('question_type', 'Unknown')}\n"
            md_content += f"- **Complexity Score:** {query_analysis.get('complexity_score', 0):.2f}\n"
            md_content += f"- **Word Count:** {query_analysis.get('word_count', 0)}\n"
            
            # Add identified entities
            entities = query_analysis.get('entities', [])
            if entities:
                md_content += f"- **Identified Entities:** {', '.join(entities)}\n"
                
            # Add enhanced query if available
            if result.get('enhanced_query'):
                md_content += f"\n**Enhanced Query:**\n> {result.get('enhanced_query')}\n"
        ...
        if answer_quality:
            md_content += "## Answer Quality\n"
            md_content += f"- **Overall Score:** {answer_quality.get('overall_score', 0):.2f}\n"
            md_content += f"- **Query Coverage:** {answer_quality.get('query_coverage', 0):.2f}\n"
            
            # Add quality suggestions
            suggestions = answer_quality.get('suggestions', [])
            if suggestions:
        ...
        # Add full context that was fed to the LLM if available
        if result.get('full_context'):
            md_content += "## Full Context Provided to LLM\n\n"
            full_context = result.get('full_context', '')
            # Display first 2000 characters with an indicator if it's truncated
            if len(full_context) > 2000:
                md_content += f"```\n{full_context[:2000]}...\n```\n\n"
                md_content += f"*(Context truncated for display - full length: {len(full_context)} characters)*\n\n"
            else:
                md_content += f"```\n{full_context}\n```\n\n"
...
"""
            html_content += f"""
                    <div class="metadata-item">
                        <strong>Question Type:</strong> {query_analysis.get('question_type', 'Unknown')}
                    </div>
                    <div class="metadata-item">
                        <strong>Complexity Score:</strong> {query_analysis.get('complexity_score', 0):.2f}
                    </div>
                    <div class="metadata-item">
                        <strong>Word Count:</strong> {query_analysis.get('word_count', 0)}
                    </div>
"""
            ...
            if entities:
                html_content += f"""
                    <div class="metadata-item">
                        <strong>Entities:</strong> {', '.join(entities)}
                    </div>
"""
            html_content += """
                </div>
...
"""
            ...
            <div class="query-box" style="margin-top: 15px;">
                <strong>Enhanced Query:</strong><br>
                {result.get('enhanced_query')}
            </div>
"""
...
"""
            html_content += f"""
                    <div class="metadata-item">
                        <strong>Overall Score:</strong> {answer_quality.get('overall_score', 0):.2f}
                    </div>
                    <div class="metadata-item">
                        <strong>Query Coverage:</strong> {answer_quality.get('query_coverage', 0):.2f}
                    </div>
                    <div class="metadata-item">
                        <strong>Has Structure:</strong> {'Yes' if answer_quality.get('has_structure') else 'No'}
                    </div>
                    <div class="metadata-item">
                        <strong>Has Attribution:</strong> {'Yes' if answer_quality.get('has_attribution') else 'No'}
                    </div>
"""
            html_content += """
                </div>
...
"""
        ...
        if result.get('full_context'):
            ...
            # Display first 2000 characters with an indicator if it's truncated
            if len(full_context) > 2000:
                display_context = full_context[:2000] + "..."
                context_note = f"<p><em>Context truncated for display - full length: {len(full_context)} characters</em></p>"
            else:
